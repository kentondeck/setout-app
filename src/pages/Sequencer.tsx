...
const JOBS: Job[] = [
  // ─── Site setout — first thing on a bare section ─────────────────────────
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Photograph everything before machinery arrives', body: 'Wide shots showing all four boundary pegs, your marker pegs, and a permanent reference (kerb, existing dwelling, power pole). Time-stamped. Save them against the job in Setout (Jobs → Photos) so they\'re still there in 12 months when someone questions the setback.', watchFor: 'Diggers and delivery trucks flatten pegs. Your photos are the only defence if one goes missing.' },
      ],
    },
    au: {
  ...
  // ─── Framing & structure ─────────────────────────────────────────────────
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Set out joist positions on the bearers', body: 'From one end, step off at the design c/c (450, or 400 / 600 per plan) across the run. Keep every centre consistent.', watchFor: 'Joist positions must match the plan — the plan sets the c/c and the location of every joist for a reason.' },
        { title: 'Lay joists across bearers', body: 'Place joists across bearers, single-span end-to-end or lap-joined over a bearer with 300 mm minimum overlap. Rotate bowed joists BOW UP — gravity pulls them straight once loaded.', watchFor: 'A joist laid bow-down will sit bow-down forever, giving a permanent dip in the floor above it.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Set out joist positions on the bearers', body: 'From one end, step off at the design c/c (450, or 600 per plan) across the run. Keep every centre consistent.', watchFor: 'Joist positions must match the plan — the plan sets the c/c and the location of every joist for a reason.' },
        { title: 'Lay joists across bearers', body: 'Place joists across bearers, single-span or lap-joined over a bearer with 300 mm minimum overlap. Rotate bowed joists BOW UP.', watchFor: 'A joist laid bow-down stays bow-down, giving a permanent dip in the floor above.' },
  ...
  },
  {
    ...
    nz: {
      ...
      materials: ['Studs, plates, nogs: 90×45 SG8 kiln-dried radiata', 'Lintels: 190×45 (or larger) LVL13 or similar per NZS 3604 §8.5 tables', 'Trimmer + jack studs at each opening (doubled per plan)', '90×3.15 flat-head bright framing nails', 'Brace panels: 12 mm plywood, or engineered brace units per NZS 3604 §5.4', 'Wall wrap: E2/AS1-compliant house wrap (Thermakraft, Watergate, Tyvek)', 'Flashing tape for opening jambs / heads (not for lapping wrap joins)', 'Bottom-plate fixings: 100 mm bugle screws to timber, Dynabolts to slab'],
      steps: [
        ...
        { title: 'Mark brace panel positions', body: 'Chalk / pencil the panel extents on the studs + plates so you (or the next person) can\'t miss where they go. Panels themselves are usually fixed AFTER the wall is stood + plumb — see the install-brace-panels job for the fixing patterns.', watchFor: 'A wall standing without its brace panels is only held by the temporary props. Get panels on before end of day, or you\'re gambling on the wind.' },
        ...
        { title: 'Fix wall wrap over the frame', body: 'Roll wrap horizontally across the wall, starting at the bottom, lapping upper courses over lower by 150 mm minimum. Staple to studs at ~300 mm c/c. The 150 mm lap sheds water on its own — no tape on the lap.', watchFor: 'E2/AS1 requires specific opening detail: sill flashing tape goes UNDER wrap, jamb tape OVER wrap, head flashing on TOP. Wrong sequence = water gets behind cladding.' },
      ],
    },
    au: {
      tools: ['Tape', 'Chalk line', 'Pencil', 'Combination square', 'Spirit level (1.8 m+)', 'Circular / drop saw', 'Hammer or nail gun', 'Cordless drill', 'Plumb bob', 'String line', 'Staple gun (for wrap)'],
      materials: ['Studs, plates, noggins: 90×45 MGP10 pine — H2-blue treated in termite zones', 'Lintels: 190×45 (or larger) LVL13 or similar per AS 1684.2 lintel tables', 'Trimmer + jack studs at each opening (doubled per plan)', '90×3.15 flat-head bright framing nails', 'Brace panels: 12 mm plywood, or engineered brace panels per AS 1684.2', 'Wall wrap: pliable membrane per AS 4200 (Enviroseal, Thermakraft, Tyvek)', 'Flashing tape for opening jambs / heads (not for lapping wrap joins)', 'Bottom-plate fixings: 100 mm Type 17 to timber, Dynabolts to slab'],
      steps: [
        ...
        { title: 'Cut plates + set out', body: 'Chalk floor, cut top + bottom plates, stack and mark stud + trimmer + jack positions on both at once. External load-bearing walls typically 450 or 600 c/c per plan.', watchFor: 'Mark brace panel extents in a different colour so you know where the panels have to land once the wall is stood.' },
        ...
        { title: 'Mark brace panel positions', body: 'Chalk / pencil the panel extents on the studs + plates so you (or the next person) can\'t miss where they go. Panels themselves are usually fixed AFTER the wall is stood + plumb — see the install-brace-panels job for the fixing patterns.', watchFor: 'A wall standing without its brace panels is only held by the temporary props. Get panels on before end of day.' },
        ...
        { title: 'Fix wall wrap over the frame', body: 'Roll wrap horizontally across the wall, starting at the bottom, lapping upper courses over lower by 150 mm minimum. Staple at ~300 mm c/c. The 150 mm lap sheds water on its own — no tape on the lap.', watchFor: 'NCC Vol 2 3.5 requires specific opening detail: sill tape UNDER wrap, jamb tape OVER wrap, head flashing on TOP. Wrong sequence = water behind cladding.' },
      ],
    },
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        },
        {
          ...
          body: 'Stack the two plates edge-to-edge. From one end, step off at the design c/c across both at once — 600 for standard non-loadbearing, 400 if loadbearing or carrying a long horizontal gib join (per NZS 3604 §8).',
        ...
        {
          title: 'Add nogs',
          body: 'Cut and install rows of nogs between studs at your desired height (usually to line up with horizontal gib joins). Nogs stop studs twisting and give a fix for the linings.',
    ...
    au: {
      ...
      steps: [
        ...
        {
          title: 'Mark stud positions on both plates',
          body: 'Stack the two plates edge-to-edge. From one end, step off at the design c/c across both at once — 600 for MGP10 non-loadbearing, 450 if loadbearing or carrying a long horizontal plasterboard join (per AS 1684.2).',
        ...
        {
          title: 'Add noggins',
          body: 'Cut and install a row of noggins between studs at roughly mid wall-height (1100–1200 mm for a standard 2.4 m ceiling). Noggins stop studs twisting and give a fixing line for horizontal plasterboard joins.',
  ...
  },
  {
    ...
    category: 'framing',
    label: 'Install brace panels (GIB Braceline / plywood)',
    summary: 'Fixing patterns for GIB Braceline + structural plywood in place.',
    nz: {
      tools: ['Tape', 'Chalk line / pencil', 'Circular saw (for ply)', 'Utility knife (for GIB)', 'Cordless drill / driver', 'Hammer or nail gun', 'Ladder / trestle'],
      materials: ['GIB Braceline 10 mm sheet (internal bracing), OR', '12 mm structural plywood (H3.2-treated for external / cavity)', 'GIB Grabber screws (32 mm) for Braceline, OR', '50×3.15 flat-head galv nails (or 50 mm ring-shank) for ply', 'Bracing schedule (BUs required per wall) from the plan'],
      steps: [
        { title: 'Read the bracing schedule', body: 'Every consented house has a bracing calc that lists panel type, size, position, and BU (Bracing Units) rating per wall. Confirm you\'re fitting the panel the schedule calls for — GIB Braceline BLG, BLP, etc. all have different BU ratings.', watchFor: 'Swapping a spec\'d GIB Braceline for a Standard sheet drops the BU rating to near zero. If the schedule says Braceline, only Braceline gives the rating.' },
        { title: 'Cut the panel to fit', body: 'Panel spans between top plate and bottom plate, edge on a stud each side. Cut with a utility knife (GIB) or circular saw (ply). Full sheet where possible — small offcut infills lose bracing capacity.', watchFor: 'Panels less than 300 mm wide don\'t count in most bracing systems. Read the fine print of the BU rating for the panel you\'re using.' },
        { title: 'Position + tack in place', body: 'Lift into position against the stood + plumb wall. Tack a couple of nails / screws to hold — enough to free your hands, not enough to lock alignment yet. Check panel is dead against every stud + plate.', watchFor: 'Panel with a gap behind a stud won\'t transfer load. Push hard to close gaps before nailing off.' },
        { title: 'GIB Braceline: nail off with the 50/50/50/75/75/150 pattern', body: 'From each corner of the panel, screws / nails at 50 mm, then another at 50, then 50 again — 3 fastenings in the first 150 mm. Then 75 mm, then 75 again. Then 150 mm centres for the rest of the perimeter. Repeat from every corner + every panel edge. Field fixings 300 mm c/c on intermediate studs.', watchFor: 'The tight corner pattern is what gives Braceline its rating. Miss the first 3 fastenings at 50 mm each and the whole panel drops to almost nothing on BUs.' },
        { title: 'Plywood: nail off perimeter 150 c/c + intermediate 300 c/c', body: 'Per NZS 3604 §5.4 for 12 mm ply: nails at 50 mm from panel edge, then 150 mm c/c around the whole perimeter (top plate, bottom plate, both edge studs). Field fixings at 300 mm c/c on intermediate studs the panel crosses.', watchFor: 'Nails too close to the edge (< 12 mm) split the ply. Nails too far apart on the perimeter drops the BU rating in a linear way — 200 mm c/c is not "close enough".' },
        { title: 'Count the nails / screws before knock-off', body: 'On any inspected job, the certifier / building inspector will count fastenings on random panels. If they find one short, they\'ll count them all. Walk the wall + count before you sign off.', watchFor: 'A NOT / rectification for under-nailed brace panels means pulling GIB / linings + re-fixing. Two minutes counting saves half a day of rework.' },
        { title: 'Photograph each panel', body: 'Wide shot showing the whole panel + nailing pattern before it gets covered. Label the wall + panel type in the shot. Log them against the job in Setout (Jobs → Photos) so they\'re all in one place when the inspector asks.', watchFor: 'A folder of unlabelled brace-panel photos is nearly useless. Use the comment on each photo in Setout to note the wall ID + panel ref — so you can still tell them apart in 6 months.' },
      ],
    },
    au: {
      ...
      steps: [
        ...
        { title: 'Count the nails before knock-off', body: 'On any inspected job, the certifier / building surveyor will count fastenings on random panels. If they find one short, they\'ll count them all. Walk the wall + count before signing off.', watchFor: 'Under-nailed brace panels = rectification order = pull linings + re-nail. Two minutes counting saves half a day of rework.' },
  ...
  },
  {
    ...
    nz: {
      ...
      materials: ['Rafters: 190×45 or 240×45 SG8 H1.2 (per NZS 3604 §10.4 rafter tables)', 'Ridge board: same depth as rafters × 25 or 45 mm thick', '90×3.15 flat-head bright framing nails', 'Collar ties or ceiling joists (roof tie)', 'Rafter tie-down straps per wind zone (NZS 3604 §10.3)'],
      steps: [
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Straighten top chord line', body: 'Run a string between the peaks of the two end trusses — that\'s your reference for a straight ridge. Sight the top of every intermediate truss against the string and adjust the temporary bracing to bring any that are off-line into plane.', watchFor: 'Once permanent bracing + straps go on, the line is locked in. Get the top chord dead straight before any of the fixings below get nailed off.' },
  ...
  },
  {
    ...
    nz: {
      ...
      materials: ['Battens: 70×45 SG8 H3.2 (typical for metal roof)', '100 mm bugle screws or 90×3.15 hot-dipped galv nails', 'Roofing underlay (breather-type, self-supporting): Vapor Barrier, Sisalation, or similar per E2/AS1', 'Underlay staples or clout nails', 'Edge closure / end caps per roofing spec'],
      steps: [
  ...
  // ─── Roofing & cladding — get the building weather-tight ─────────────────
  {
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Cut fascia + mitre corners', body: 'Cut fascia to length between corners. Mitre external corners at 45° (or use a corner box) — unless the barge board runs past to cover the gutter ends, in which case butt-cut the fascia so the barge caps it. Prime any cut ends of timber fascia.', watchFor: 'Gapped mitre reads sloppy forever. Test-fit before nailing.' },
        { title: 'Fix fascia to rafter tails', body: 'Timber fascia: 2× countersunk screws per rafter tail (top + bottom), pre-drilled + countersunk so the heads sit below the surface for filling before paint. COLORBOND fascia: spec\'d clips + screws.', watchFor: 'One fixing per tail lets fascia bow between rafters. Two — top + bottom.' },
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Position first sheet at eave', body: 'Run a string line along the eave, offset from the bottom edge of where the sheet will sit — that\'s your alignment reference for every sheet across the roof. Square the first sheet to the string. Overhang past fascia line by 50 mm (varies with profile). If a valley or hip is on the same side, cut-fit at that end first.', watchFor: 'A crooked first sheet cascades across the roof. Get it square to the string before screwing.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Position first sheet at eave', body: 'Run a string line along the eave, offset from the bottom edge of where the sheet will sit — that\'s your alignment reference for every sheet across the roof. Square the first sheet to the string. Overhang past fascia by 50 mm (varies with profile). Cut-fit at valley / hip end first if present.', watchFor: 'Crooked first sheet cascades across roof. Square to the string before screwing.' },
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Fix sheets to ribbon plate + fascia', body: 'Push sheet up between ribbon plate and fascia. HardieSoffit (fibre-cement) is nailed — galv FC nails per Hardie spec. Any other soffit material (ply, PVC, cedar lining) follow the manufacturer\'s fixing spec.', watchFor: 'Overdriven FC nails crack the sheet + create a moisture path. Nail head sits flush or slightly proud — never sunk below the surface.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Fix sheets to ribbon plate + fascia', body: 'Push sheet up between ribbon plate + fascia. HardieSoffit (fibre-cement) is nailed — galv FC nails per Hardie spec. Any other soffit material (ply, PVC, cedar lining) follow the manufacturer\'s fixing spec.', watchFor: 'Overdriven FC nails crack the sheet + create a moisture path. Nail head sits flush or slightly proud — never sunk below the surface.' },
  ...
  },
  {
    ...
    nz: {
      ...
      materials: ['Battens: 20 mm × 45 mm H3.2 kiln-dried pine (per E2/AS1 cavity depth)', 'Fastenings per the cladding manufacturer\'s spec (through batten + wrap into stud)', 'Vermin mesh (galv or SS, ~5 mm mesh) at cavity base', 'Neutral-cure sealant'],
      steps: [
        ...
        { title: 'Check batten straightness', body: 'Once all battens are up, run a straight edge over them to pick up any high or low spots. Pack out any low batten with off-cut shims + fix off.', watchFor: 'Bowed battens telegraph through the cladding as a visible wave. Get straight before cladder starts.' },
    ...
    au: {
      tools: ['Tape', 'Drop / circular saw', 'Cordless drill / impact driver', 'Hammer', 'Utility knife', 'Sealant gun', 'Straight edge'],
      materials: ['Battens: 20 mm × 45 mm H3-treated pine (some states + wind classes 40 mm)', 'Fastenings per the cladding manufacturer\'s spec (through batten + wrap into stud)', 'Vermin mesh (galv or SS, ~5 mm mesh) at cavity base', 'Neutral-cure sealant'],
      steps: [
        ...
        { title: 'Check batten straightness', body: 'Once all battens are up, run a straight edge over them to pick up high or low spots. Pack out any low batten with shims + fix off.', watchFor: 'Bowed battens telegraph through cladding as visible waves. Straight before cladder starts.' },
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Cut the scriber to length', body: 'Floor to soffit for a full-height internal corner, or per the plan detail. Cut square + clean.', watchFor: 'Prime all four faces + cut ends BEFORE fitting. Unprimed cut ends are the #1 rot cause on weatherboard corner details.' },
  ...
  // ─── Doors & windows — installed once building is weather-tight ──────────
  {
    ...
    nz: {
      ...
      materials: ['Window unit (aluminium joinery: APL group — Vantage / Metro / Altherm)', 'Timber or plastic packers', 'Sill flashing (extruded metal, colour-matched)', 'Sill flashing tape (butyl, 300 mm wide typical)', 'Head flashing tape', 'Neutral-cure sealant (Bostik or Sika)', 'Countersunk screws through window jamb (per manufacturer)'],
      steps: [
        ...
        { title: 'Apply sill flashing tape', body: 'Butyl or self-adhesive flashing tape across the sill of the framed opening, extending up each jamb at least 250 mm. Bond the tape by hand-rolling it — no air pockets. If the plan calls for a metal sill flashing (some cavity systems + absorbent claddings), that goes in first and the tape runs over it, sloped OUT so water sheds.', watchFor: 'Cold weather (<10 °C) stops butyl tape adhering. Warm the surface with a heat gun or postpone.' },
        { title: 'Position window in opening', body: 'Lift into opening, sit on sill packers to bring to correct RL. Set the window into the opening so the interior face of the jamb sits 10 mm proud of the framing — that gives the gib a flush finish against the reveal. Plumb + level in both directions.', watchFor: 'Windows out of level bind on the sash rails. Get level within 2 mm before screwing off.' },
        ...
        { title: 'Tape the head flashing to the wrap', body: 'Once the head flashing is over the top of the window, run flashing tape along its top edge and up onto the wall wrap — that seals the top of the flashing so water running down the wrap gets diverted out over the flashing, not behind it.', watchFor: 'Untaped head flashing = water tracks down the wrap and behind the flashing. The tape at the top edge is what stops that.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Apply sill flashing tape', body: 'Butyl or self-adhesive flashing tape across the sill, up each jamb 250 mm min. Hand-roll to bond. If the plan calls for a metal sill flashing (some cavity systems + absorbent claddings), that goes in first with the tape running over it, sloped OUT.', watchFor: 'Cold weather (<10 °C) stops butyl adhering. Warm with heat gun or postpone.' },
        { title: 'Position window in opening', body: 'Lift into opening, sit on sill packers to correct RL. Set the window into the opening so the interior face of the jamb sits 10 mm proud of the framing — that gives the plasterboard a flush finish against the reveal. Plumb + level both directions.', watchFor: 'Out-of-level windows bind on the sash rails. Level within 2 mm before screwing.' },
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Plumb hinge-side jamb + fix', body: 'Long spirit level on hinge-side jamb. Pack + plumb both faces. Fix through jamb into stud with countersunk screws (pre-drilled, heads sunk below the surface for filling before paint) — one behind each hinge, one at top, one at bottom (avoid the strike-height area, that gets cut out later).', watchFor: 'Plumb ONE face at a time, both must be plumb. A jamb plumb in one plane and tilted in the other means the door sags to one side when opened.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Plumb hinge-side jamb + fix', body: 'Long spirit level on hinge-side jamb. Pack + plumb both faces. Fix through jamb into stud with countersunk screws (pre-drilled, heads sunk below the surface for filling before paint) — one behind each hinge, top, bottom (avoid the strike-height area, gets cut out later).', watchFor: 'Plumb ONE face at a time — both faces must be plumb. Plumb one plane + tilted other = door sags to one side.' },
  ...
  },
  {
    ...
    nz: {
      ...
      steps: [
        ...
        { title: 'Pack + fix latch-side jamb', body: 'Close door. Pack latch-side jamb to a consistent 3 mm gap all the way top to bottom. Fix through packers with countersunk screws — one at top, middle, and latch strike height.', watchFor: 'Inconsistent latch gap looks amateur and can catch the door. 3 mm dead all the way.' },
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Pack + fix latch-side jamb', body: 'Close door. Pack latch-side to 3 mm consistent gap top to bottom. Fix with countersunk screws at top, middle, latch height.', watchFor: 'Inconsistent gap looks amateur + can catch. 3 mm dead all the way.' },
  ...
  // ─── Decking & outdoor — usually last on a new build ─────────────────────
  {
    ...
    au: {
      ...
      steps: [
        ...
        { title: 'Lay joists over bearers', body: 'Joist spacing per board thickness — 400 c/c for 32 mm, 450 c/c for 40 mm. Skew-nail 2× 90 mm per bearing, or joist hangers where joist doesn\'t sit on bearer.', watchFor: 'Bow UP. Bow-down joist gives permanent dip in deck.' },
