...
export function AddToJobPrompt({ calculationId }: AddToJobPromptProps) {
  ...
  // Once a job is linked, `entry.jobId` flips true the instant handleCreateAndAdd
  // runs — the very same render that also sets `toast`. An early return here
  // for the "already linked" branch meant that render swapped straight to the
  // confirmation view and skipped past the <Toast/> below entirely, so it
  // never got a chance to show. Toast is rendered unconditionally instead —
  // it already no-ops on a null message — so it survives whichever branch
  // the rest of the component is in.
  return (
    <>
      {linkedJob ? (
        <div
